...
export const resetPassword=async (req,res)=>{
    ...
    try{
        ...
        if(!resetPassword){
            const message="Invalid password reset link. Try again";
            return res.redirect(`/users/resetpassoword?status=error&message=${message}`);
        ...
        if(expiresAt<Date.now())
            {
                const message="Reset Passowrd link has been expired. Please try again";
                res.redirect(`/users/resetpassword?status=error&message=${message}`);

            }else{
                const isMatch=await compareString(token,resetToken);
            if(!isMatch){
                const message="Invalid reset password link.Please try again later";
                res.redirect(`/users/resetpassword?status=error&message=${message}`);
